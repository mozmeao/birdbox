...
const NewsletterForm = {
  ...
  validateFields: () => {
    ...
    if (!FormUtils.checkEmailValidity(email)) {
      NewsletterForm.handleFormError(FormUtils.errorList.EMAIL_INVALID_ERROR);
      return false;
    }
    ...
    if (newsletters.length === 0) {
      NewsletterForm.handleFormError(FormUtils.errorList.NEWSLETTER_ERROR);
      return false;
    }

    // Confirm privacy policy is checked
    if (!privacy) {
      NewsletterForm.handleFormError(FormUtils.errorList.PRIVACY_POLICY_ERROR);
      return false;
    }

    // Terms checkbox only appears on /firefox/ios/testflight/ page.
    if (terms && !terms.checked) {
      NewsletterForm.handleFormError(FormUtils.errorList.LEGAL_TERMS_ERROR);
      return false;
    }
